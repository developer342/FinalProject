...
import com.example.finalapp.appui.HomeActivity;
    ...
    btnLogin.setOnClickListener(v -> {
      ...
      authRepo.login(new LoginRequest(email, pw), new Callback<TokenResponse>() {
        ...
        public void onResponse(Call<TokenResponse> call, Response<TokenResponse> resp) {
          ...
          // ✅ 로그인 성공 → 토큰 저장
          TokenResponse tokenRes = resp.body();
          TokenStore.saveTokens(tokenRes.getAccessToken(), tokenRes.getRefreshToken());

          // ✅ 내 정보 조회 후 홈으로 이동
          userRepo.me(new Callback<UserProfileResponse>() {
            @Override
            public void onResponse(Call<UserProfileResponse> c, Response<UserProfileResponse> r) {
              btnLogin.setEnabled(true);
